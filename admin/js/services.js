...
const ServicesPage = (function(){
    ...
    if(items.length===0){
      ...
      items.forEach(s=>{
        html += `<div class="item-card">
          ...
          <div class="item-card-body"><h3>${s.title}</h3><p>${s.desc||''}</p>
  ...
  function openForm(id){
    ...
    showModal(`
      ...
      <div class="modal-body">
        ...
        <div class="form-group"><label>Description</label><textarea class="form-input" id="svDesc">${s.desc||''}</textarea></div>
  ...
  function save(id){
    ...
    Store.set('services',items); closeModal(); render();
    showToast(id?'Service updated!':'Service added!');
  }

  function remove(id){
    if(!confirm('Delete?')) return;
    Store.set('services', Store.get('services',[]).filter(s=>s.id!==id)); render();
